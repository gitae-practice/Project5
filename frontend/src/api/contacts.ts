...
export const getContacts = () => api.get<ContactSummary[]>('/contacts').then((r) => r.data)

// 교집합 비교용: 취향 포함 전체 목록
export const getContactsFull = () => api.get<Contact[]>('/contacts/full').then((r) => r.data)

export const getContact = (id: number) => api.get<Contact>(`/contacts/${id}`).then((r) => r.data)
...
export const uploadPhoto = (contactId: number, file: File) => {
  ...
  return api
    .post<{ photoUrl: string }>(`/contacts/${contactId}/photo`, formData, {
...
export const deleteGift = (giftId: number) => api.delete(`/contacts/gifts/${giftId}`)
...
export const addMeeting = (
  contactId: number,
  data: { date: string; places?: MeetingPlaceInput[]; memo?: string },
...
export const addMeetingBulk = (data: {
  contactIds: number[]
  date: string
...
export const getPlaceRatingStats = () =>
  api
    .get<{ name: string; visitCount: number; avgRating: number | null; ratingCount: number }[]>(
      '/contacts/meeting-places/ratings',
    )
    .then((r) => r.data)
